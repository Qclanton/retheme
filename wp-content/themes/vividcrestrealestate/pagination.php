    ...
    <div class="col--right">
        <ul class="pagintaion">
            <li class="previous" <?= $pagination->current > 1 ? "" : "style='display:none'" ?>>
                <a href="<?= site_url("/properties?current={$pagination->previous}&per_page={$pagination->per_page}&sort={$pagination->sort_encoded}") ?>">
                    «
                </a>
            </li>
            <li class="page-button page-button-first <?= $pagination->current == 1 ? "current" : "" ?>">
                <a href="<?= site_url("/properties?current=1&per_page={$pagination->per_page}&sort={$pagination->sort_encoded}") ?>">
                    1
                </a>
            </li>
            ...
            <?php for ($page = ($pagination->current-2); $page <= ($pagination->current+2); $page++) { ?>
                <? if ($page > 1 && $page < $pagination->total_pages) { ?>
                    <li class="page-button <?= $page == $pagination->current ? "current" : ""?>">
                        <a href="<?= site_url("/properties?current={$page}&per_page={$pagination->per_page}&sort={$pagination->sort_encoded}") ?>">
                            <?= $page ?>
                        </a>
                    </li>
                <?php } ?>
            ...
            <li class="page-button page-button-last <?= $pagination->total_pages == $pagination->current ? "current" : ""?>" <?= $pagination->total_pages < 5 ? "style='display:none'" : "" ?>>
                <a href="<?= site_url("/properties?current={$pagination->total_pages}&per_page={$pagination->per_page}&sort={$pagination->sort_encoded}") ?>">
                    <?= $pagination->total_pages ?>
                </a>                
            </li>
            <li class="next" <?= $pagination->total_pages < 4 || $pagination->total_pages == $pagination->current ? "style='display:none'" : "" ?>>
                <a href="<?= site_url("/properties?current={$pagination->next}&per_page={$pagination->per_page}&sort={$pagination->sort_encoded}") ?>">
                    »
                </a>
           </li>
        </ul>
